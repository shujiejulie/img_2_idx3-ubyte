...
'''
MMCBNU(6000)专用
def get_num_idx3lab(labels_list, img_dir):
    sub_dir = img_dir + '\\' + str(labels_list[0])
    sub_list = makelabels.makesLabels(sub_dir)
    img_subdir = sub_dir + '\\' + str(sub_list[0])

    imglist = os.listdir(img_subdir)
    aImg = Image.open(img_subdir + '\\' + imglist[0]).convert('L')

    num = len(labels_list) * len(sub_list) * len(imglist)
    rows = aImg.size[1]
    columns = aImg.size[0]
...
def readImage_idx3lab(imgU_name, labels_list, img_dir, num, rows, columns):
    ...
    file.write(struct.pack('>I', columns))
    for i in xrange(len(labels_list)):
        sub_dir = img_dir + '\\' + str(labels_list[i])
        sub_list = makelabels.makesLabels(sub_dir)
        sub_list.sort()

        for m in xrange(len(sub_list)):
            img_subdir = sub_dir + '\\' + str(sub_list[m])
            imglist = os.listdir(img_subdir)
            imglist.sort()
